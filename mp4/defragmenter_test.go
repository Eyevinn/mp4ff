...
// buildDefragDescendingChain writes n one-sample fragments followed by n-1
// two-sample fragments whose tfdt walks back one sample each, voiding the
// records above every cut. The abandoned tail is never re-declared, so the
// file rejects with a coverage error after all cuts are resolved.
func buildDefragDescendingChain(tb testing.TB, n int) []byte {
	tb.Helper()
	buf, add := startDefragChainFile(tb)
	for k := 0; k < n; k++ {
		add(uint64(k)*512, 1, 512)
	}
	for j := 1; j < n; j++ {
		add(uint64(n-1-j)*512, 2, 512)
	}
	return buf.Bytes()
}

// buildDefragPaddedRewind writes n one-sample fragments separated by 1-tick
// tfdt gaps (padding every boundary), then n two-sample fragments each
// rewinding one sample, so every cut runs against the padding bookkeeping.
func buildDefragPaddedRewind(tb testing.TB, n int) []byte {
	tb.Helper()
	buf, add := startDefragChainFile(tb)
	for k := 0; k < n; k++ {
		add(uint64(k)*513, 1, 512)
	}
	cut := uint64(n-1) * 513
	for j := 0; j < n; j++ {
		add(cut, 2, 513)
...
// startDefragChainFile writes a one-video-track init and returns the buffer
// plus a function appending a fragment of count dur-tick samples at dts.
func startDefragChainFile(tb testing.TB) (*bytes.Buffer, func(dts uint64, count int, dur uint32)) {
	tb.Helper()
	init := mp4.CreateEmptyInit()
	init.Moov.Mvhd.Timescale = 600
	trak := init.AddEmptyTrack(defragVideoTimescale, "video", "und")
	...
	seqNr := uint32(0)
	data := defragPayload(1, 130)
	return &buf, func(dts uint64, count int, dur uint32) {
		seqNr++
		...
		for s := 0; s < count; s++ {
			frag.AddFullSample(mp4.FullSample{
				Sample:     mp4.Sample{Flags: defragSyncFlags(), Dur: dur, Size: uint32(len(data))},
				DecodeTime: dts + uint64(s)*uint64(dur), Data: data,
			})
		}
		if err := frag.Encode(&buf); err != nil {
			tb.Fatal(err)
...
// overlapping input: every shape must stay within the same order as the
// non-overlapping control. The descending shape ends in a coverage error
// by construction; all cuts are still resolved before it surfaces.
func BenchmarkDefragmentOverlapChain(b *testing.B) {
	for _, mode := range []struct {
		name  string
		build func(tb testing.TB, n int) []byte
		errOK bool
	}{
		{name: "chain", build: func(tb testing.TB, n int) []byte { return buildDefragOverlapChain(tb, n, true) }},
		{name: "control", build: func(tb testing.TB, n int) []byte { return buildDefragOverlapChain(tb, n, false) }},
		{name: "descending", build: buildDefragDescendingChain, errOK: true},
		{name: "padded-rewind", build: buildDefragPaddedRewind},
	} {
		b.Run(mode.name, func(b *testing.B) {
			data := mode.build(b, 5000)
			...
			for i := 0; i < b.N; i++ {
				...
				var out bytes.Buffer
				err = mp4.DefragmentTracks(f, rs, &out, 1)
				if err != nil && (!mode.errOK || !strings.Contains(err.Error(), "never re-declared")) {
					b.Fatal(err)
				}
			}
		})
...
// TestDefragmentBoundaryCutInPaddingKeepsCutoff pins that a cut landing
// exactly on a record's boundary inside later gap padding does not advance
// the record's cutoff into the padding: the content its earlier trim
// abandoned is still unrecovered and the file must reject.
func TestDefragmentBoundaryCutInPaddingKeepsCutoff(t *testing.T) {
	var buf bytes.Buffer
	init := createDefragInit(t)
	if err := init.Encode(&buf); err != nil {
		t.Fatal(err)
	}
	// A declares [0, 4096) in 8 samples.
	addDefragFragment(t, &buf, 1, 1, defragSampleRun(0, 8, 1, 10))
	// A sample-less traf rewinds to 2048: A is trimmed, cutoff 2048.
	moof := &mp4.MoofBox{}
	_ = moof.AddChild(mp4.CreateMfhd(2))
	traf := &mp4.TrafBox{}
	_ = moof.AddChild(traf)
	_ = traf.AddChild(mp4.CreateTfhd(1))
	_ = traf.AddChild(mp4.CreateTfdt(2048))
	trun := mp4.CreateTrun(0)
	trun.Flags &^= mp4.TrunDataOffsetPresentFlag
	_ = traf.AddChild(trun)
	if err := moof.Encode(&buf); err != nil {
	...
	// B at 3072 pads A's last kept sample across [1536, 3072).
	addDefragFragment(t, &buf, 3, 1, defragSampleRun(3072, 2, 101, 10))
	// C rewinds to 2560, A's boundary inside the padding, covering to 4096.
	addDefragFragment(t, &buf, 4, 1, defragSampleRun(2560, 3, 201, 10))
	out, err := defragment(t, buf.Bytes(), 1)
	if err == nil || !strings.Contains(err.Error(), "never re-declared") {
		t.Errorf("boundary cut in padding gave %v, want a coverage error for A's [2048, 2560)", err)
	}
	if out.Len() != 0 {
		t.Errorf("wrote %d bytes before rejecting input", out.Len())
	}
