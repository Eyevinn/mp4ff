...
func validateTrackEdits(track *defragTrack) error {
	if track.origin == 0 || track.trak.Edts == nil {
		return nil
	}
...
func rebaseTrackEdits(track *defragTrack, mediaDur uint64, movieTs uint32) {
	...
	for _, elst := range trak.Edts.Elst {
		if elst.Version == 0 && elstNeeds64Bits(elst) {
			elst.Version = 1
		}
	}
}

func elstNeeds64Bits(elst *ElstBox) bool {
...
// insertEdts adds edts to the trak right after tkhd, matching the
// conventional box order.
func insertEdts(trak *TrakBox, edts *EdtsBox) {
	trak.Edts = edts
	for i, child := range trak.Children {
		if _, ok := child.(*TkhdBox); ok {
			children := make([]Box, 0, len(trak.Children)+1)
			children = append(children, trak.Children[:i+1]...)
...
func fillSampleTables(track *defragTrack) error {
	...
	stsc := &StscBox{}
	var lastSdi uint32
	for i, chunk := range track.chunks {
	...
	stsz := &StszBox{SampleNumber: uint32(len(track.samples))}
	uniform := len(track.samples) > 0
	for _, s := range track.samples {
		if s.size != track.samples[0].size || s.size == 0 {
			uniform = false
			break
		}
	}
	if uniform {
		stsz.SampleUniformSize = track.samples[0].size
	} else {
		stsz.SampleSize = make([]uint32, len(track.samples))
